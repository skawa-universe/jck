...
function B(b){x(b).then(function(d){var e=null;console.log("File size in characters: "+d.length);var c=0;try{for(var h=document.querySelector(".error");null===e||e<d.length;)e=w(d,e),++c;console.log("Number of records: "+c);h.classList.remove("hidden");var g=b.name;h.textContent=g?g+" is correct":"Correct";h.classList.add("correct")}catch(f){if(f instanceof t){console.log("Number of records before the error: "+c);h.classList.remove("correct");h.classList.remove("hidden");for(g=c=e=0;g<f.position;++g)"\n"===
