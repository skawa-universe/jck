...
g);h.classList.remove("correct");for(b=f=d=0;b<k.position;++b)"\n"===e.charAt(b)?(++d,f=0):++f;b=c.name;g="Error";b&&(g+=" in "+b);h.textContent=g+" at position "+k.position+" row "+(d+1)+" column "+(f+1);b=document.createElement("pre");d=k.position-240;0>d&&(d=0);f=d+480;f>e.length&&(f=e.length);f=e.substring(d,f);e=y(f.substring(0,k.position-d),0);d=y(f.substring(k.position-d),(e[e.length-1]||"").length);b.appendChild(z(e.map(function(a){return Array.from(a).map(A).join("")}).join("\n"),"before"));
